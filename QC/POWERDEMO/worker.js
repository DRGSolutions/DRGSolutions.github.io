  ...
  try {
    ...
    if (msg.type === "preview") {
      ...
      for (const [nid, n] of Object.entries(nodes)) {
        ...
        const csInfo = getConstructionStandardInfo(n);
        const displayName = makeDisplayName(scid, poleTag);
        const nodeColorHex = getNodeColorHex(job, n, options.nodeColorAttribute);
        poleOut.push({ poleId: nid, scid, poleTag, constructionStandard: csInfo.value, constructionStandardExists: csInfo.exists, displayName, lat, lon, nodeColorHex });
      }
      ...
      for (const c of candidates) {
        ...
        if (options.includeGuys) {
          ...
          for (const gid in guys) {
            ...
            // Row output (QC)
            const tr = traces[traceId] || {};
            const owner = (tr && tr.company != null) ? String(tr.company) : "";
            const label = (tr && (tr.label || tr.name)) ? String(tr.label || tr.name) : "Guy";
            const row = makeRow({ traceId, category: "Guy", owner, existingIn: h, proposedIn: h + move, comments: label });
            const sortIn = (row && row._sortIn != null) ? Number(row._sortIn) : Number(h);
...
polesOut.push({
        ...
        scid,
        poleTag,
        constructionStandard: csInfo.value,
        constructionStandardExists: csInfo.exists,
        lat,
        lon,
        nodeColorHex,
...
function extractAttrValue(v) {
  if (v == null) return "";
  if (typeof v === "string" || typeof v === "number" || typeof v === "boolean") return String(v);
  if (typeof v === "object") {
    if (Object.prototype.hasOwnProperty.call(v, "value")) return extractAttrValue(v.value);
    const first = getFirstValue(v);
    if (first != null && first !== v) return extractAttrValue(first);
  }
  return "";
}

function getConstructionStandardInfo(n) {
  const attrs = (n && n.attributes) ? n.attributes : {};
  const want = normalizeAttrKey("Construction_Standard");
  let found = false;
  let raw = null;

  for (const [k, v] of Object.entries(attrs)) {
    if (!k) continue;
    if (normalizeAttrKey(k) === want) {
      found = true;
      raw = v;
      break;
    }
  ...
  if (!found) return { exists: false, value: "" };
  const value = extractAttrValue(raw).trim();
