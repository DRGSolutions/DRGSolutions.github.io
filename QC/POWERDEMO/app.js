...
(() => {
  ...
  const DEFAULT_RULES = {
    // Top-level enables (both disabled by default; cards are also collapsed by default in the UI)
    commsEnabled: false,
    powerEnabled: false,
    powerUtility: "",
    pole: {
  ...
  const els = {
    ...
    // Rules cards
    btnToggleCommsRules: $("btnToggleCommsRules"),
    commsRulesBody: $("commsRulesBody"),
    commsRulesFieldset: $("commsRulesFieldset"),
    enableCommsRules: $("enableCommsRules"),

    btnTogglePowerRules: $("btnTogglePowerRules"),
    powerRulesBody: $("powerRulesBody"),
  ...
  // Power rules state (client-only; loaded from .rls static files)
  const powerRulesCache = new Map(); // utilityName -> { ok: bool, config?: object, error?: string }
  let powerRulesActive = {
    utilityName: "",
    utilityKey: "",
    config: null,
    standardsByKey: new Map(), // UPPER construction standard -> rules object
  };
  ...
  function init() {
    ...
    // Populate Power Rules dropdown before applying stored selection.
    populatePowerUtilityDropdown();

    applyRulesToUi();
    applyRulesEnabledStateToUi();
    // Ensure any enabled power utility rules are loaded immediately.
    onRulesUiChange();
      ...
      btn.addEventListener("click", () => {
        ...
        if (tab === "map") {
          setTimeout(() => {
            ...
            if (viewMode === "2d" && map) {
              map.invalidateSize();
              try { updateConstructionStandardCallouts(); } catch (_) {}
            }
            if (viewMode === "3d") resizeThreeRenderer();
          }, 50);
        }
...
function onPointerDown(e) {
  ...
  function setViewMode(mode) {
    ...
    if (viewMode === "3d") {
      ...
      if (map) {
        ...
        // Also refresh Construction Standard callouts so permanent tooltips do not appear offset.
        setTimeout(() => {
  ...
  function rebuildThreeScene() {
    ...
    // Poles
    {
      ...
      for (const p of (model.poles || [])) {
        ...
        // 3D Construction Standard callout badge.
        // This is separate from QC issues and is driven by the pole's Construction_Standard attribute.
        // NOTE: In 3D view, use "*!*" / "*?*" to keep the visible glyph centered in the badge.
        try {
          if (threeState.labelLayer) {
            const csState = getPoleConstructionStandardState(p, { requirePowerEnabled: true });
            if (csState && (csState.state === "missing" || csState.state === "invalid")) {
              const b = document.createElement("div");
              const cls = ["three-cs-badge", csState.state === "missing" ? "three-cs-badge--missing" : "three-cs-badge--invalid"];
              b.className = cls.join(" ");
              const mark = (csState.state === "missing") ? "*!*" : "*?*";
              b.textContent = mark;
              b.title = (csState.state === "missing")
                ? "CONSTRUCTION STANDARD MISSING"
                : "INVALID CONSTRUCTION STANDARD";
              threeState.labelLayer.appendChild(b);

              // Slightly above the SCID label so both remain readable.
              const worldPos = new THREE.Vector3(pos.x, hFt + 5.6, pos.z);
              threeState.csBadges3d.set(poleId, { el: b, poleGroup: group, worldPos });
            }
          }
        } catch (_) {}
      }
  ...
  function startThreeLoop() {
    ...
    const tick = (now) => {
      ...
      // 3D HTML overlays must be re-projected each frame.
      try { updateThreeScidLabels(); } catch (_) {}
      try { updateThreeConstructionStandardBadges(); } catch (_) {}
  ...
  function updateThreeConstructionStandardBadges() {
    // 3D-only visualization: show compact Construction Standard callouts
    // ("!" missing, "?" invalid) above poles.
    if (!threeState || !threeState.camera || !threeState.renderer) return;
    if (!threeState.csBadges3d || !(threeState.csBadges3d instanceof Map)) return;
    if (!threeState.labelLayer) return;

    const show = !!(rules && rules.powerEnabled);
    const cam = threeState.camera;
    const w = threeState.renderer.domElement.clientWidth;
    const h = threeState.renderer.domElement.clientHeight;

    for (const rec of threeState.csBadges3d.values()) {
      if (!rec || !rec.el || !rec.worldPos) continue;
      const el = rec.el;
      const poleGroup = rec.poleGroup;

      // Hide if power rules are disabled or the pole is filtered out.
      if (!show || (poleGroup && poleGroup.visible === false)) {
        el.style.display = "none";
        continue;
      }

      // Project into screen space.
      const v = rec.worldPos.clone();
      v.project(cam);
  ...
  function initEvents() {
    ...
    // Rules card collapse/expand (UI only)
    if (els.btnToggleCommsRules && els.commsRulesBody) {
      els.btnToggleCommsRules.addEventListener("click", () => {
        toggleRuleSection(els.commsRulesBody, els.btnToggleCommsRules);
      });
    }
    if (els.btnTogglePowerRules && els.powerRulesBody) {
      els.btnTogglePowerRules.addEventListener("click", () => {
        toggleRuleSection(els.powerRulesBody, els.btnTogglePowerRules);
      });
    }
    ...
    const ruleInputs = [
      // Top-level enables
      els.enableCommsRules,
      els.enablePowerRules,
      els.powerUtilitySelect,
  ...
  function getPowerUtilityList() {
    const list = (typeof window !== "undefined" && window.POWER_UTILITY_LIST) ? window.POWER_UTILITY_LIST : [];
    return Array.isArray(list) ? list.filter((x) => x && typeof x === "object") : [];
  }

  function resolvePowerRulesFile(utilityName) {
    const want = normalizeOwnerKey(utilityName);
    if (!want) return "";
    for (const entry of getPowerUtilityList()) {
      const nm = entry && entry.name != null ? String(entry.name) : "";
      if (normalizeOwnerKey(nm) === want) return String(entry.rulesFile || "");
    }
    return "";
  }

  function populatePowerUtilityDropdown() {
    const sel = els.powerUtilitySelect;
    if (!sel) return;

    const list = getPowerUtilityList();
    sel.innerHTML = "";

    const optNone = document.createElement("option");
    optNone.value = "";
    optNone.textContent = "— (select utility)";
    sel.appendChild(optNone);
    ...
    const want = String((rules && rules.powerUtility) ? rules.powerUtility : "").trim();
    sel.value = want;
    if (sel.value !== want) sel.value = "";
  }

  async function ensurePowerRulesActive(utilityName) {
    const nm = String(utilityName || "").trim();
    const uKey = normalizeOwnerKey(nm);
    if (!uKey) {
      powerRulesActive = { utilityName: "", utilityKey: "", config: null, standardsByKey: new Map() };
      return powerRulesActive;
    }

    // Fast path if already active
    if (powerRulesActive && powerRulesActive.utilityKey === uKey && powerRulesActive.config) return powerRulesActive;

    if (powerRulesCache.has(uKey)) {
      const cached = powerRulesCache.get(uKey);
      if (cached && cached.ok && cached.config) {
        powerRulesActive = buildPowerRulesActiveState(nm, cached.config);
        return powerRulesActive;
      }
    }

    const rulesFile = resolvePowerRulesFile(nm);
    if (!rulesFile) {
      powerRulesCache.set(uKey, { ok: false, error: "No rules file mapped" });
      powerRulesActive = { utilityName: nm, utilityKey: uKey, config: null, standardsByKey: new Map() };
      return powerRulesActive;
    }

    // NOTE: This app is designed to run serverless, including from file://.
    // Fetching local static files can fail under file:// due to browser security.
    // To keep the project backend-free, we support an embedded fallback via
    // <script type="application/json" id="embeddedAsset_<file>">...</script>.
    async function loadRulesText(path) {
      // 1) Try fetch (works when hosted via http(s)).
      try {
        const resp = await fetch(`./${path}`, { cache: "no-store" });
        if (resp && resp.ok) return await resp.text();
      } catch (_) {}

      // 2) Fallback to embedded JSON (works under file://).
      try {
        const embId = embeddedAssetIdForPath(path);
        const el = document.getElementById(embId);
        const txt = el ? String(el.textContent || "").trim() : "";
        if (txt) return txt;
      } catch (_) {}

      // 3) Final fallback: attempt XHR (some environments allow it).
      try {
        const txt = await new Promise((resolve, reject) => {
          ...
          xhr.onreadystatechange = () => {
            ...
            if (xhr.status === 200 || xhr.status === 0) resolve(String(xhr.responseText || ""));
      ...
      throw new Error("Unable to load rules file (fetch + embedded fallback failed)");
    }

    try {
      const txt = await loadRulesText(rulesFile);
      const parsed = JSON.parse(txt);
      powerRulesCache.set(uKey, { ok: true, config: parsed });
      powerRulesActive = buildPowerRulesActiveState(nm, parsed);
      return powerRulesActive;
    } catch (err) {
      const msg = (err && err.message) ? err.message : String(err);
      powerRulesCache.set(uKey, { ok: false, error: msg });
      powerRulesActive = { utilityName: nm, utilityKey: uKey, config: null, standardsByKey: new Map() };
      logLine(`[Power Rules] Failed to load ${rulesFile}: ${msg}`);
      return powerRulesActive;
    }
  }

  function buildPowerRulesActiveState(utilityName, config) {
    const nm = String(utilityName || "").trim();
    const uKey = normalizeOwnerKey(nm);
    const standardsByKey = new Map();

    const raw = (config && typeof config === "object" && config.constructionStandards && typeof config.constructionStandards === "object")
  ...
  function getPowerConstructionStandardValidationContext() {
    const utility = String((rules && rules.powerUtility) ? rules.powerUtility : "").trim();
    const wantUtilityKey = normalizeOwnerKey(utility);
    const activeOk = !!(wantUtilityKey && powerRulesActive && powerRulesActive.config && powerRulesActive.utilityKey === wantUtilityKey);
    const standards = activeOk && powerRulesActive && powerRulesActive.standardsByKey ? powerRulesActive.standardsByKey : new Map();
    return { utility, wantUtilityKey, activeOk, standards };
  }

  function getPoleConstructionStandardState(pole, opts = {}) {
    const requirePowerEnabled = (opts && typeof opts.requirePowerEnabled === "boolean") ? opts.requirePowerEnabled : true;
    const ctx = getPowerConstructionStandardValidationContext();
    const csInfo = poleConstructionStandardInfo(pole);
    const csRaw = String(csInfo && csInfo.value != null ? csInfo.value : "").trim();
    const csKey = csRaw ? csRaw.toUpperCase() : "";

    if (!csInfo || !csInfo.exists || !csRaw) {
      return { state: "missing", csRaw, csKey, canValidate: false, ctx };
    }

    const canValidate = !!(ctx && ctx.activeOk && (!requirePowerEnabled || (rules && rules.powerEnabled)));
    if (canValidate && ctx.standards && !ctx.standards.has(csKey)) {
      return { state: "invalid", csRaw, csKey, canValidate: true, ctx };
    }
    return { state: "ok", csRaw, csKey, canValidate, ctx };
  }
  ...
  function updateConstructionStandardCallouts() {
    // Show Construction_Standard callouts only when Power Rules are enabled.
    const powerEnabled = !!(rules && rules.powerEnabled);

    // If Power Rules are off, remove any existing Construction Standard tooltips immediately.
    if (!powerEnabled) {
      for (const marker of poleMarkers.values()) {
        if (!marker) continue;
        const tt = marker.getTooltip ? marker.getTooltip() : null;
        const isOurTooltip = !!(tt && tt.options && typeof tt.options.className === "string" && tt.options.className.includes("cs-tooltip"));
        if (isOurTooltip) {
    ...
    // Avoid binding/updating Leaflet tooltips while the 2D map is hidden (e.g., user is in 3D view or another tab).
    // Leaflet will compute pixel positions against a hidden container, which can appear offset until the user moves the map.
    const mapEl = document.getElementById("map");
    const mapVisible = (viewMode === "2d") && mapEl && !mapEl.classList.contains("is-hidden") && mapEl.offsetParent !== null;
    if (!mapVisible) return;

    const selUtility = String((rules && rules.powerUtility) ? rules.powerUtility : "").trim();
    const wantUtilityKey = normalizeOwnerKey(selUtility);

    const standards = (powerRulesActive && powerRulesActive.standardsByKey instanceof Map)
      ? powerRulesActive.standardsByKey
      : new Map();

    const canValidate = powerEnabled && wantUtilityKey && powerRulesActive && powerRulesActive.config && powerRulesActive.utilityKey === wantUtilityKey;

    const list = (model && Array.isArray(model.poles) && model.poles.length)
      ? model.poles
      : (Array.isArray(previewPoles) ? previewPoles : []);

    for (const p of list) {
      const poleId = String(p && p.poleId != null ? p.poleId : "");
      if (!poleId) continue;
      const marker = poleMarkers.get(poleId);
      if (!marker) continue;
      ...
      const csInfo = poleConstructionStandardInfo(p);
      const csRaw = String(csInfo.value || "").trim();
      const csKey = csRaw ? csRaw.toUpperCase() : "";

      let msg = "";
      let cls = "cs-tooltip";

      // Map callouts are intentionally compact:
      // - Missing Construction Standard => "*!*"
      // - Invalid Construction Standard => "*?*"
      if (!csInfo.exists || !csRaw) {
        msg = "*!*";
        cls = "cs-tooltip cs-tooltip--missing";
      } else if (canValidate && !standards.has(csKey)) {
        msg = "*?*";
        cls = "cs-tooltip cs-tooltip--invalid";
      }
      ...
      // Bind/update a permanent tooltip (acts like a dialog box with an arrow).
      try {
        const curContent = (tt && tt.getContent) ? String(tt.getContent() ?? "") : "";
        const curClass = (tt && tt.options && typeof tt.options.className === "string") ? tt.options.className : "";
        const needsRebind = !isOurTooltip || curContent !== msg || curClass !== cls;
        if (needsRebind) {
          if (isOurTooltip) marker.unbindTooltip();
        ...
        // Force a position update so tooltips snap to the marker immediately.
        const nt = marker.getTooltip ? marker.getTooltip() : null;
        if (nt && typeof nt.update === "function") {
          try { nt.update(); } catch (_) {}
        }
      } catch (_) {}
    }

    // One more deferred update pass to avoid initial-frame offsets.
    try {
      setTimeout(() => {
        try {
          for (const marker of poleMarkers.values()) {
            ...
            const tt = marker.getTooltip ? marker.getTooltip() : null;
            const isOurTooltip = !!(tt && tt.options && typeof tt.options.className === "string" && tt.options.className.includes("cs-tooltip"));
            if (isOurTooltip && tt && typeof tt.update === "function") {
              try { tt.update(); } catch (_) {}
            }
          }
        } catch (_) {}
      }, 0);
  ...
  function runQcEngine(model, rules) {
    ...
    const commsEnabled = !!(rules && rules.commsEnabled);
    const powerEnabled = !!(rules && rules.powerEnabled);

    // Only use the active power ruleset if it matches the selected utility.
    const wantUtilityKey = normalizeOwnerKey(rules && rules.powerUtility ? rules.powerUtility : "");
    const powerCtx = (powerEnabled && powerRulesActive && powerRulesActive.config && normalizeOwnerKey(powerRulesActive.utilityName) === wantUtilityKey)
      ? powerRulesActive
      : null;
    ...
    for (const pole of poles) {
      const base = commsEnabled ? evaluatePole(pole, rules) : { status: "pass", issues: [] };

      // Power rules are additive (and independently enable/disable-able).
      const pow = powerEnabled ? evaluatePowerPole(pole, rules, powerCtx) : { issues: [] };
      const mergedIssues = [...(base.issues || []), ...(pow.issues || [])];

      const res = {
        status: deriveStatus(mergedIssues),
        issues: mergedIssues,
      };
    ...
    for (const ms of midspans) {
      const res = commsEnabled ? evaluateMidspan(ms, rules) : { status: "pass", issues: [] };
  ...
  // ────────────────────────────────────────────────────────────────────────────
  //  Power rules (utility + Construction_Standard driven)
  // ────────────────────────────────────────────────────────────────────────────

  function poleConstructionStandardInfo(pole) {
    // IMPORTANT:
    // Some exports include an explicit boolean for attribute existence.
    // However, we treat any non-empty value as "exists" even if that boolean is false,
    // so that invalid Construction_Standard values are correctly classified as INVALID
    // (not missing).
    const value = pole && pole.constructionStandard != null ? String(pole.constructionStandard).trim() : "";
    let exists = false;
    if (value) {
      exists = true;
    } else if (pole && typeof pole.constructionStandardExists === "boolean") {
      exists = !!pole.constructionStandardExists;
    } else {
      exists = !!(pole && pole.constructionStandard != null);
    }
    return { exists, value };
  }

  function evaluatePowerPole(pole, rules, powerCtx) {
    const issues = [];
    if (!pole) return { status: "pass", issues };

    const poleId = String(pole.poleId || "");
    const poleName = poleDisplayName(pole, poleId);

    const csInfo = poleConstructionStandardInfo(pole);
    const csRaw = String(csInfo.value || "").trim();

    // When Power Rules are enabled, Construction_Standard validity is itself a QC requirement.
    // - Missing => FAIL
    // - Invalid (not defined in the selected utility ruleset) => WARN
    if (!csInfo.exists || !csRaw) {
      issues.push(issue(
        "FAIL",
        "pole",
        ...
        poleName,
        "POWER.CS.MISSING",
        "Construction Standard is missing for this pole.",
        { constructionStandard: "", csState: "missing" }
      ));
      return { status: deriveStatus(issues), issues };
    }

    const csKey = csRaw.toUpperCase();

    // If the utility ruleset isn't loaded yet, we can't validate the standard or apply
    // Construction_Standard-specific rules.
    if (!powerCtx || !powerCtx.config || !powerCtx.standardsByKey) {
      return { status: deriveStatus(issues), issues };
    }

    const std = powerCtx.standardsByKey.get(csKey);
    if (!std || typeof std !== "object") {
      issues.push(issue(
        ...
        poleName,
        "POWER.CS.INVALID",
        `Construction Standard "${csRaw}" is not valid for ${powerCtx.utilityName}.`,
        { constructionStandard: csKey, csState: "invalid" }
      ));
      return { status: deriveStatus(issues), issues };
    }

    const stdRules = (std && std.rules && typeof std.rules === "object") ? std.rules : std;
    ...
    const primaries = atts.filter(a => a._cls && a._cls.kind === "power_primary" && getH(a) != null);
    const neutralsSecondaries = atts.filter(a => a._cls && (a._cls.kind === "power_neutral" || a._cls.kind === "power_secondary") && getH(a) != null);
    const transformers = atts.filter(a => String(a.category || "").toLowerCase() === "equipment" && a._cls && a._cls.isTransformer && getH(a) != null);

    // Only evaluate guy/down-guy rules for the selected power utility.
    const utilityKey = normalizeOwnerKey(powerCtx.utilityName);
    const companyGuys = atts.filter(a => {
      if (!a || !a._cls) return false;
      const isGuyLike = a._cls.kind === "guy" || a._cls.isGuy || a._cls.isDownGuy;
      if (!isGuyLike) return false;
      if (getH(a) == null) return false;
      return normalizeOwnerKey(a._cls.owner) === utilityKey;
    });

    // 1) Primary and (highest) Neutral/Secondary separation minimum.
    const rPrimNs = stdRules.primaryToNeutralSecondarySeparation;
    if (rPrimNs && rPrimNs.enabled) {
      const minIn = Number(rPrimNs.minIn);
      if (Number.isFinite(minIn) && primaries.length && neutralsSecondaries.length) {
        const lowPrimary = Math.min(...primaries.map(getH));
        const highNs = Math.max(...neutralsSecondaries.map(getH));
        const sep = lowPrimary - highNs;
        if (sep < minIn) {
          const pAtt = primaries.filter(a => getH(a) === lowPrimary);
          const nsAtt = neutralsSecondaries.filter(a => getH(a) === highNs);
          issues.push(issue(
            "FAIL",
            "pole",
            poleId,
            poleName,
            "POWER.PRIMARY_NS_SEP",
            `Construction Standard ${csKey}: separation between the lowest primary and the highest neutral or secondary is ${fmtFtIn(sep)} (minimum ${fmtFtIn(minIn)}).`,
            { constructionStandard: csKey, separationIn: sep, minRequiredIn: minIn, attachmentIds: attachmentIdsOf(pAtt, nsAtt) }
          ));
        }
      }
    }

    // 2) Neutral and Secondaries must be below Transformers.
    const rNsBelowXfmr = stdRules.neutralSecondaryBelowTransformer;
    if (rNsBelowXfmr && rNsBelowXfmr.enabled) {
      if (transformers.length && neutralsSecondaries.length) {
        const tBottom = Math.min(...transformers.map(getH));
        for (const w of neutralsSecondaries) {
          const wh = getH(w);
          if (wh == null) continue;
          if (wh >= tBottom) {
            issues.push(issue(
              ...
              poleName,
              "POWER.NS_BELOW_XFMR",
              `Construction Standard ${csKey}: neutral or secondary conductor is not below the transformer (neutral/secondary at ${fmtFtIn(wh)} vs transformer at ${fmtFtIn(tBottom)}).`,
              { constructionStandard: csKey, transformerIn: tBottom, wireIn: wh, attachmentIds: attachmentIdsOf(transformers, w) }
            ));
          }
        }
      }
    ...
    // 3) Power Company Guys/Down Guys must be at least X inches away from Neutrals/Secondaries.
    const rGuySep = stdRules.guySeparationToNeutralSecondary;
    if (rGuySep && rGuySep.enabled) {
      const minIn = Number(rGuySep.minIn);
      if (Number.isFinite(minIn) && companyGuys.length && neutralsSecondaries.length) {
        const nsHeights = neutralsSecondaries.map(getH).filter((v) => v != null);
        for (const g of companyGuys) {
          const gh = getH(g);
          if (gh == null) continue;
          let best = Infinity;
          let bestNs = null;
          for (const ns of neutralsSecondaries) {
            const nh = getH(ns);
            if (nh == null) continue;
            const d = Math.abs(gh - nh);
            if (d < best) { best = d; bestNs = ns; }
          }
          if (best < minIn) {
            issues.push(issue(
              "FAIL",
              ...
              poleName,
              "POWER.GUY_NS_SEP",
              `Construction Standard ${csKey}: ${powerCtx.utilityName} guy/down-guy is ${fmtFtIn(best)} from the nearest neutral or secondary (Neutrals and Secondaries) (minimum ${fmtFtIn(minIn)}).`,
              { constructionStandard: csKey, minRequiredIn: minIn, separationIn: best, attachmentIds: attachmentIdsOf(g, bestNs) }
            ));
          }
        }
      }
    ...
    // 4) Guys/Down Guys must be above Neutral/Secondaries.
    const rGuyAbove = stdRules.guyAboveNeutralSecondary;
    if (rGuyAbove && rGuyAbove.enabled) {
      if (companyGuys.length && neutralsSecondaries.length) {
        const nsTop = Math.max(...neutralsSecondaries.map(getH));
        for (const g of companyGuys) {
          const gh = getH(g);
          if (gh == null) continue;
          if (gh <= nsTop) {
            issues.push(issue(
              ...
              poleName,
              "POWER.GUY_ABOVE_NS",
              `Construction Standard ${csKey}: ${powerCtx.utilityName} guy/down-guy must be above the Neutrals and Secondaries (guy at ${fmtFtIn(gh)} vs highest neutral or secondary at ${fmtFtIn(nsTop)}).`,
              { constructionStandard: csKey, guyIn: gh, nsTopIn: nsTop, attachmentIds: attachmentIdsOf(g, neutralsSecondaries) }
            ));
          }
        }
      }
    ...
    // 5) Guys/Down Guys must be below Neutral/Secondaries.
    const rGuyBelow = stdRules.guyBelowNeutralSecondary;
    if (rGuyBelow && rGuyBelow.enabled) {
      if (companyGuys.length && neutralsSecondaries.length) {
        const nsBottom = Math.min(...neutralsSecondaries.map(getH));
        for (const g of companyGuys) {
          const gh = getH(g);
          if (gh == null) continue;
          if (gh >= nsBottom) {
            issues.push(issue(
              ...
              poleName,
              "POWER.GUY_BELOW_NS",
              `Construction Standard ${csKey}: ${powerCtx.utilityName} guy/down-guy must be below the Neutrals and Secondaries (guy at ${fmtFtIn(gh)} vs lowest neutral or secondary at ${fmtFtIn(nsBottom)}).`,
              { constructionStandard: csKey, guyIn: gh, nsBottomIn: nsBottom, attachmentIds: attachmentIdsOf(g, neutralsSecondaries) }
            ));
          }
        }
      }
  ...
  function renderPoleDetails(pole) {
    ...
    // Construction Standard display (shown in Details regardless of whether Power Rules are enabled).
    const csState = getPoleConstructionStandardState(pole, { requirePowerEnabled: false });
    let csLineHtml = "";
    if (csState.state === "missing") {
      csLineHtml = `
        <div class="details-cs details-cs--missing">
          <span class="details-cs__label">Construction Standard:</span>
          <span class="details-cs__value">CONSTRUCTION STANDARD MISSING</span>
        </div>
      `;
    } else if (csState.state === "invalid") {
      csLineHtml = `
        <div class="details-cs details-cs--invalid">
          <span class="details-cs__label">Construction Standard:</span>
          <span class="details-cs__value">${escapeHtml(csState.csRaw || "")}</span>
          <span class="details-cs__flag">INVALID CONSTRUCTION STANDARD</span>
        </div>
      `;
    } else {
      csLineHtml = `
        <div class="details-cs details-cs--ok">
          <span class="details-cs__label">Construction Standard:</span>
          <span class="details-cs__value">${escapeHtml(csState.csRaw || "")}</span>
        </div>
      `;
  ...
  function clearIssuesTable() {
    const showCsCol = !!(rules && rules.powerEnabled);
    const span = showCsCol ? 6 : 5;
    els.issuesTbody.innerHTML = `<tr><td colspan="${span}" class="muted">No issues yet.</td></tr>`;
  }
  ...
  function renderIssuesTable() {
    ...
    if (!rows.length) {
      const span = showCsCol ? 6 : 5;
      els.issuesTbody.innerHTML = `<tr><td colspan="${span}" class="muted">No matching issues.</td></tr>`;
      return;
    }
    ...
    els.issuesTbody.innerHTML = rows.map((iss) => {
      ...
      let csCell = "";
      if (showCsCol && iss.entityType === "pole") {
        const p = poleById.get(String(iss.entityId || ""));
        if (p) {
          const csState = getPoleConstructionStandardState(p, { requirePowerEnabled: true });
          if (csState.state === "missing") csCell = "CONSTRUCTION STANDARD MISSING";
          else if (csState.state === "invalid") csCell = `${csState.csRaw || ""} • INVALID CONSTRUCTION STANDARD`;
          else csCell = csState.csRaw || "";
        }
      }

      const csTd = showCsCol ? `<td class="issues-col-cs">${escapeHtml(csCell)}</td>` : "";
  ...
  function coerceImportedRules(obj) {
    ...
    const merged = {
      commsEnabled: (typeof src.commsEnabled === "boolean") ? src.commsEnabled : DEFAULT_RULES.commsEnabled,
      powerEnabled: (typeof src.powerEnabled === "boolean") ? src.powerEnabled : DEFAULT_RULES.powerEnabled,
      powerUtility: (typeof src.powerUtility === "string") ? src.powerUtility.trim() : DEFAULT_RULES.powerUtility,
    ...
    // Top-level other fields
    merged.commsEnabled = !!merged.commsEnabled;
    merged.powerEnabled = !!merged.powerEnabled;
    merged.powerUtility = String(merged.powerUtility || "").trim();
  ...
  function toggleRuleSection(bodyEl, btnEl) {
    if (!bodyEl || !btnEl) return;
    const isCollapsed = bodyEl.classList.toggle("is-collapsed");
    btnEl.textContent = isCollapsed ? "▸" : "▾";
  }

  function applyRulesEnabledStateToUi() {
    if (els.enableCommsRules) els.enableCommsRules.checked = !!rules.commsEnabled;
    if (els.enablePowerRules) els.enablePowerRules.checked = !!rules.powerEnabled;
    ...
    // Ensure Construction Standard legend rows reliably show/hide in both 2D and 3D.
    // (Some environments can miss the CSS-based toggle if the body class isn't applied yet.)
    try {
      const rows = document.querySelectorAll(".legend-row--cs");
      const on = !!(rules && rules.powerEnabled);
      rows.forEach((el) => {
    ...
    // Dropdown selection
    if (els.powerUtilitySelect) {
      const want = String(rules.powerUtility || "").trim();
  ...
  function onRulesUiChange() {
    ...
    // Keep power ruleset in sync (async load from .rls static files)
    const powerOn = !!rules.powerEnabled;
    const utility = String(rules.powerUtility || "").trim();
    const utilityKey = normalizeOwnerKey(utility);

    if (!powerOn || !utilityKey) {
      powerRulesActive = { utilityName: "", utilityKey: "", config: null, standardsByKey: new Map() };
      updateConstructionStandardCallouts();
    ...
    // Recompute immediately with whatever is currently loaded (may be empty until fetch completes).
    updateConstructionStandardCallouts();
    if (model) recomputeQc();

    // Load/activate the selected utility's rules, then re-run QC + callouts.
    ensurePowerRulesActive(utility).then((ctx) => {
      // If the user changed selection while the file was loading, ignore.
      const stillOn = !!(rules && rules.powerEnabled);
      const stillKey = normalizeOwnerKey(rules && rules.powerUtility ? rules.powerUtility : "");
      if (!stillOn || stillKey !== utilityKey) return;
  ...
  function readRulesFromUi() {
    ...
    // Top-level enables
    r.commsEnabled = !!(els.enableCommsRules && els.enableCommsRules.checked);
    r.powerEnabled = !!(els.enablePowerRules && els.enablePowerRules.checked);
    r.powerUtility = String((els.powerUtilitySelect && els.powerUtilitySelect.value) ? els.powerUtilitySelect.value : "").trim();
...
function setJobName(name) {
  ...
  function fmtFtIn(inches) {
    ...
    // Guard against rounding that yields 12" (carry into feet).
    if (inch >= 12) {
      ft += 1;
      inch = 0;
    }
  ...
  function loadRules() {
    try {
      ...
      return coerceImportedRules(parsed);
    } catch (_) {
